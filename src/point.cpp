...
void Point::optimize(const size_t n_iter)
{
  ...
  for(size_t i=0; i<n_iter; i++)
  {
    ...
    for(auto it=obs_.begin(); it!=obs_.end(); ++it)
    {
      ...
      Point::jacobian_xyz2uv(p_in_f, (*it)->frame->T_f_w_.rotation_matrix(), J);//NOTE:重投影误差关于特征点三维坐标的一阶导数，即其雅可比矩阵，十四讲P165
      const Vector2d e(vk::project2d((*it)->f) - vk::project2d(p_in_f));
      new_chi2 += e.squaredNorm();//NOTE:重投影误差
      A.noalias() += J.transpose() * J;
      b.noalias() -= J.transpose() * e;
    }

    // solve linear system
    const Vector3d dp(A.ldlt().solve(b));//NOTE:高斯牛顿法解 三维坐标 增量
    ...
    // update the model
    Vector3d new_point = pos_ + dp;//NOTE:三维坐标按增量更新
    old_point = pos_;
    pos_ = new_point;
    chi2 = new_chi2;
#ifdef POINT_OPTIMIZER_DEBUG
